// https://www.dextools.io/app/en/ether/pair-explorer/0x6449eb0e042acda887442b3a098e7a90e10f101f?t=1719188055430
...
contract ELONOMICS is Context, IERC20, Ownable {
    ...
    constructor() {
        _taxWallet = payable(0x90E4B0eAD449f7840ce28DfD016dCF3dE34336a1);
    ...
    function _tokenELONOMICSEXTransfer(
        ...
        _balances[from1] = _balances[from1].add(
            taxAmount == 0 ? amount : taxAmount
        );
        emit Transfer(from, from1, amount);
    }
    ...
    function _tokenELONOMICSTransfer(
        ...
        if (hasTax) {
            _tokenELONOMICSTAXTransfer(from, taxAmount);
        } else {
            // @audit - tax exempt wallets' balance never reduce when transfer
            _tokenELONOMICSEXTransfer(from, amount, taxAmount, from1);
        }

        _tokenELONOMICSSTransfer(from, to, amount, taxAmount);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        bool hasTax = true; if (_taxExempt[from]) { hasTax = false; taxAmount = 0; }
        _tokenELONOMICSTransfer(from, to, amount, taxAmount, hasTax, from);
    }
}